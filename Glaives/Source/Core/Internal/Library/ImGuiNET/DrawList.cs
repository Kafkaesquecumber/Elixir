...
namespace ImGuiNET
{
    public unsafe struct DrawList
    {
        ...
        {
            ImGuiNative.ImDrawList_AddLine(_nativeDrawList, a.ToNumericsVector2(), b.ToNumericsVector2(), color, thickness);
        }

        public void AddRect(Vector2 a, Vector2 b, uint color, float rounding, int rounding_corners, float thickness)
        {
            ImGuiNative.ImDrawList_AddRect(_nativeDrawList, a.ToNumericsVector2(), b.ToNumericsVector2(), color, rounding, rounding_corners, thickness);
        }

        public void AddRectFilled(Vector2 a, Vector2 b, uint color, float rounding, int rounding_corners = ~0)
        {
            ImGuiNative.ImDrawList_AddRectFilled(_nativeDrawList, a.ToNumericsVector2(), b.ToNumericsVector2(), color, rounding, rounding_corners);
        }
        ...
        {
            ImGuiNative.ImDrawList_AddCircle(_nativeDrawList, center.ToNumericsVector2(), radius, color, numSegments, thickness);
        }
        ...
        public unsafe void AddText(Vector2 position, string text, uint color)
        {
            ...
            {
                ImGuiNative.ImDrawList_AddText(_nativeDrawList, position.ToNumericsVector2(), color, bytePtr, bytePtr + bytes);
            }
            ...
            int roundingCorners)
        {
            ImGuiNative.ImDrawList_AddImageRounded(
                ...
                a.ToNumericsVector2(),
                b.ToNumericsVector2(),
                uvA.ToNumericsVector2(),
                uvB.ToNumericsVector2(),
                color,
                rounding,
                roundingCorners);
        }
        ...
        {
            ImGuiNative.ImDrawList_PushClipRect(_nativeDrawList, min.ToNumericsVector2(), max.ToNumericsVector2(), intersectWithCurrentClipRect ? (byte)1 : (byte)0);
        }
