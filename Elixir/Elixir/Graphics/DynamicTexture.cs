...
namespace Elixir.Graphics
{
    ...
    public class DynamicTexture : IDisposable
    {
        ...
        public DynamicTexture(string file)
        {
            if (!File.Exists(file))
            {
                throw new ElixirException($"Failed to create dynamic texture: file '{file}' not found");
            }
        ...
        public DynamicTexture(Texture texture)
        {
            if (texture.Handle == 0)
            {
                throw new ElixirException("Failed to create dynamic texture from texture: The texture has been disposed");
            }
